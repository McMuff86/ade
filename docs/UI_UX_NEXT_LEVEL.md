...
| Raum noch nicht ausgeliefert | Aufgaben/Notizen zeigen eine ehrliche Beschreibung und „wird gerade umgesetzt“, statt einer leeren Fläche. | – |
...
**Nicht auf diesem Branch** (bewusst, wegen paralleler Arbeit): der Inhalt
der Räume Aufgaben/Notizen. Er entsteht im Arbeitsbaum `main` (siehe
[TASKS_NOTES.md](TASKS_NOTES.md)). Dieser Branch reserviert die Räume und
rendert dort einen Platzhalter (`renderer/nav/RoomPlaceholder.tsx`, Tablet:
`Empty` in `main.tsx`).

### Zusammenführen mit der Tasks/Notes-Arbeit

Erwartete Konflikte, alle mechanisch:

1. `src/shared/appViews.ts`: beide Seiten fügen `tasks`/`notes` ein; die
   deutschen Labels dieses Branches übernehmen.
2. `src/renderer/App.tsx`: den Platzhalter-Zweig durch den
   `DesktopOrganizer`-Zweig ersetzen; die Kopfzeile dieses Branches behalten.
3. `src/mobile/main.tsx`: den `Empty`-Platzhalter durch `MobileOrganizer`
   ersetzen; Kopf und Werkzeugzeile dieses Branches behalten.
4. Tests der Organizer-Arbeit, die `'Tasks'`/`'Notes'`/„Neue Aufgabe“ als
   Tab- oder Dialognamen verwenden, auf „Aufgaben“/„Notizen“ umstellen. In der
   Organizer-Oberfläche heisst die persönliche Aufgabe weiterhin „Neue
   Aufgabe“; das ist jetzt eindeutig, weil der Agentenauftrag „Agent
   beauftragen“ heisst.

Danach `RoomPlaceholder.tsx` löschen.
...
als Ganzes steht noch aus und gehört zur Zusammenführung auf `main`.
...
- Tasks/Notes-Inhalt einhängen (siehe oben) und die Speicherzustände aus dem
  Briefing („Auf diesem Gerät gespeichert“, „Mit PC abgeglichen“,
