...
| Independent project workspaces | Main-service foundation verified on native Windows | Bounded configured-root discovery, exact original/linked checkout identity, atomic registration without an agent or injected instructions. No UI/API launch integration yet; that follows goal T2b/T3 |
...
| Mobile project entry | Native Windows flow verified | Projekte → workspace preparation → independent Codex/Claude/Grok/Shell choice; no CLI starts before the explicit launch action. New Project uses the configured native parent and a Codex profile. WSL home access remains a separate agent flow. See `PROJECT_ENTRY.md` |
