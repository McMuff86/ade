...
export async function supervisionFlow(page: Page, sessions: Array<{ id: string; repositoryId: string; name: string }>, check: (name: string, ok: boolean) => void, tablet?: Page) {
  ...
  await page.getByRole('dialog', { name: 'Einstellungen', exact: true }).waitFor();
  ...
  await page.getByRole('dialog', { name: 'Gespräche', exact: true }).locator('#conversation-mode-project').click();
  // The button opens the conversation modes first; project supervision is one of them.
  const dialog = page.getByRole('dialog', { name: 'Projektbetreuung', exact: true });
  ...
  const mobile = tablet.getByRole('dialog', { name: 'Projektbetreuung', exact: true });
  ...
  await projectDialog.getByRole('button', { name: 'Projektbetreuung', exact: true }).click();
  ...
  check('closing project supervision restores its project opener', await projectDialog.getByRole('button', { name: 'Projektbetreuung', exact: true }).evaluate(node => node === document.activeElement));
}
