...
require(${JSON.stringify(resolve('out/main/index.js'))});`);
  ...
  check('slower computer default appears in the settings', await section.getByRole('slider', { name: 'Tempo', exact: true }).inputValue() === '0.85');
  await section.getByRole('slider', { name: 'Tempo', exact: true }).fill('0.8');
  await section.getByRole('slider', { name: 'Stabilität', exact: true }).fill('0.6');
  await section.getByLabel('Speaker Boost', { exact: true }).uncheck();
  await section.getByRole('button', { name: 'Stimme testen', exact: true }).click();
  await section.getByText('Stimmtest vollständig abgespielt.', { exact: true }).waitFor();
  const preview = JSON.parse(readFileSync(join(root, 'request.json'), 'utf8')).voice_settings;
  check('unsaved voice preview uses native provider parameters', preview.speed === 0.8 && preview.stability === 0.6 && preview.use_speaker_boost === false);
  ...
  check('Computer greeting uses the saved speed', JSON.parse(readFileSync(join(root, 'request.json'), 'utf8')).voice_settings.speed === 0.8);
  await page.getByRole('tab', { name: 'Allgemein', exact: true }).click();
  await page.getByRole('tab', { name: 'Stimme', exact: true }).click();
  await section.getByRole('button', { name: 'Stimmen laden', exact: true }).click();
  check('saved speed survives leaving the tab', await section.getByRole('slider', { name: 'Tempo', exact: true }).inputValue() === '0.8');
  await section.getByRole('button', { name: 'Ruhiger Computer', exact: true }).click();
  check('preset is a reversible draft', await section.getByRole('slider', { name: 'Tempo', exact: true }).inputValue() === '0.85' && (await page.evaluate(() => window.ade.invoke('config:get'))).settings.speechTuning?.speed === 0.8);
  await section.getByRole('button', { name: 'Änderungen verwerfen', exact: true }).click();
  check('discard restores saved parameters', await section.getByRole('slider', { name: 'Tempo', exact: true }).inputValue() === '0.8');
  await page.setViewportSize({ width: 960, height: 650 });
