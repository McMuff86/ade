...
`);
  ...
  await page.getByText('Zum Neustarten dieses Gerät am PC unter Einstellungen → Verbundene Geräte freigeben.', { exact: true }).waitFor();
  ...
  check('page reload keeps the original paired identity', await page.getByRole('button', { name: 'Agent beauftragen', exact: true }).isVisible());
