...
void (async () => {
  ...
  const quotaFixture = `if (cli == "CODEX" && args.Length > 0 && args[0] == "app-server") { string line; while ((line = Console.ReadLine()) != null) {
...
`);
  ...
  await workspace.getByLabel('Terminalanzeige', { exact: true }).waitFor();
  await desktop.getByRole('button', { name: 'Neue Sitzung', exact: true }).click();
  ...
  await desktop.keyboard.press('Escape');
  check('cancel launch restores focus to plus', await desktop.getByRole('button', { name: 'Neue Sitzung', exact: true }).evaluate((node) => node === document.activeElement));
  ...
  await desktop.getByRole('button', { name: 'Neu starten', exact: true }).click();
