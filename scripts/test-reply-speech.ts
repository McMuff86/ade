...
void (async () => {
  ...
  await refuses('provider failure is bounded', () => replies.speak('desktop:1', failed.replyId), /HTTP 500/);
  const failedCount = requests; failure = false;
  await refuses('a failed paid request is never silently retried', () => replies.speak('desktop:1', failed.replyId), /HTTP 500/);
