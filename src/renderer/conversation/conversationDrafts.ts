...
export class ConversationDrafts {
  ...
  constructor(private readonly storage: Pick<Storage, 'getItem' | 'setItem'>, scope: string) {
    if (!/^[A-Za-z0-9:._-]{1,160}$/.test(scope)) throw new Error('Ungültiger Gesprächsspeicher.');
  ...
  read(): State {
    ...
    try {
      ...
      return s;
    } catch { throw new Error('Gesprächsentwürfe können nicht gelesen werden. Der gespeicherte Stand bleibt erhalten; es wird nichts gesendet.'); }
  }
  private save(state: State): void {
    const raw = JSON.stringify(state);
    if (raw.length > MAX_CHARS || state.drafts.length > 64 || state.recordings.length > 16) throw new Error('Gesprächsentwürfe sind voll. Nicht mehr benötigten Text in einem anderen Gespräch leeren.');
    this.storage.setItem(this.key, raw);
  }
  select(id: string): void {
    if (id !== '' && !conversationId(id)) throw new Error('Ungültiges Gespräch.');
    const s = this.read(); s.selected = id; this.save(s);
  }
  text(id: string): string {
    if (!id) return '';
    if (!conversationId(id)) throw new Error('Ungültiges Gespräch.');
    ...
    if (legacy !== null && !conversationText(legacy)) throw new Error('Gespeicherter Gesprächsentwurf ist ungültig.');
    return legacy ?? '';
  }
  edit(id: string, text: string): void {
    if (!conversationId(id) || !conversationText(text)) throw new Error('Gesprächsentwurf ist ungültig oder zu lang.');
  ...
  reserve(command: PendingConversationCommand): void {
    if (!validConversationCommand(command) || command.operation !== 'create' && command.operation !== 'send') throw new Error('Ungültiger Gesprächsauftrag.');
    ...
    if (old) { if (JSON.stringify(old) !== JSON.stringify(command)) throw new Error('Zuerst den vorherigen Vorgang mit derselben ID prüfen.'); return; }
    s.pending.push(command); this.save(s);
  }
  acknowledge(command: PendingConversationCommand, receipt: ConversationReceipt): void {
    if (command.operation === 'send' && receipt.conversationId !== command.conversationId || !conversationId(receipt.conversationId)) throw new Error('Gesprächsquittung gehört nicht zum Auftrag.');
    const s = this.read();
    const index = s.pending.findIndex(c => c.commandId === command.commandId);
    if (index < 0) return;
    if (JSON.stringify(s.pending[index]) !== JSON.stringify(command)) throw new Error('Gesprächsauftrag wurde inzwischen geändert.');
  ...
  saveRecording(value: ConversationRecordingDraft, prepare = false): void {
    if (!conversationId(value.id) || !conversationId(value.jobId) || !recordingText(value.text) || typeof value.complete !== 'boolean') throw new Error('Ungültiger Diktatentwurf.');
    const s = this.read(); const old = s.recordings.find(r => r.id === value.id);
    if (prepare ? !!old : !old || old.jobId !== value.jobId) throw new Error('Der Diktatentwurf wurde in einer anderen Ansicht geändert.');
  ...
  consumeRecording(id: string, jobId: string): string {
    ...
    if (!value?.text.trim() || s.pending.some(c => c.operation === 'send' && c.conversationId === id)) throw new Error('Diktat ist leer oder die letzte Nachricht wartet noch auf Bestätigung.');
    const old = this.text(id); const text = old + (old && !old.endsWith('\n') ? '\n' : '') + value.text;
    if (!conversationText(text)) throw new Error('Nachricht würde zu lang. Nachricht zuerst kürzen; das Diktat bleibt erhalten.');
