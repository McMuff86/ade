...
export function ConversationVoice({ id, drafts, port, enabled, onApply }: {
  id: string; drafts: ConversationDrafts; port: ConversationRecordingPort; enabled: boolean; onApply(text: string): void;
}) {
  ...
  return <div className="conversation-voice" role="group" aria-label="Diktat für ADE" ref={region} tabIndex={-1}>
    <p className="conversation-note">Diktieren über ElevenLabs. Der Text bleibt zur Prüfung hier; „An ADE senden“ startet die Nachricht. Beim Wechsel endet die Aufnahme. Bis zu fünf Minuten je Diktat.</p>
    {view.phase === 'idle' && <button type="button" disabled={!enabled} onClick={() => act(() => { void controller.current?.start(); })}>Nachricht diktieren</button>}
    ...
    {view.value && <>
      ...
      {!active && <div className="conversation-controls">
        <button type="button" disabled={!enabled || !view.value.text.trim()} onClick={() => act(() => { const text = controller.current?.apply(); if (text !== undefined) onApply(text); })}>Diktat in Nachricht übernehmen</button>
        <button type="button" disabled={!enabled} onClick={() => act(() => { void controller.current?.recover(); })}>Vorheriges Diktat prüfen</button>
        <button type="button" disabled={!enabled} onClick={() => act(() => controller.current?.discard())}>Diktat verwerfen</button>
      </div>}
    </>}
