...
export const SESSION_LAUNCH_LABELS: Record<SessionLaunchChoice['mode'], string> = {
  shell: 'Leeres Terminal', agent: 'Gespeichertes Agent-Profil', codex: 'Codex', hermes: 'Hermes', ollama: 'Ollama',
};
export function validSessionChoice(value: Record<string, unknown>): boolean {
  return ['shell', 'agent', 'codex', 'hermes', 'ollama'].includes(String(value.mode))
