  ...
  return <section className="m-continue" aria-labelledby="continue-title">
    ...
    {error ? <p role="status">{error}</p> : !inventory ? <p role="status">Sitzungen werden geladen…</p> : !inventory.sessions.length
      ...
      : <ul className="m-session-cards">{inventory.sessions.map((session) => <li key={session.id}><button disabled={host.status !== 'online'}
        onClick={(event) => { event.currentTarget.focus(); onSession(session); }} aria-label={`Weiterarbeiten in ${session.terminalHome ? 'Benutzerverzeichnis' : session.projectName ?? host.catalog?.repositories.find((repo) => repo.id === session.repositoryId)?.name ?? 'Eigener Workspace'} mit ${session.terminalHome ? 'Ohne Agent-Profil' : session.projectWorkspaceId ? session.launchProfileName ?? 'Ohne Agent-Profil' : host.catalog?.agents.find((agent) => agent.id === session.agentId)?.name ?? 'Agent'} · ${session.title}`}>
        <strong>{session.terminalHome ? 'Freies Terminal' : session.projectName ?? host.catalog?.repositories.find((repo) => repo.id === session.repositoryId)?.name ?? 'Eigener Workspace'}</strong>
        <span>{session.terminalHome ? 'Ohne Agent-Profil' : session.projectWorkspaceId ? session.launchProfileName ?? 'Ohne Agent-Profil' : host.catalog?.agents.find((agent) => agent.id === session.agentId)?.name} · {session.title}{session.branch ? ` · ${session.branch}` : ''}</span>
