...
export function MobileOrganizer({ host, access, kind, onKind, onRun }: { host: MobileHost; access: MobileHostState | null; kind: OrganizerKind; onKind(kind: OrganizerKind): void; onRun(id: string): void }) {
  ...
  const port = useMemo<OrganizerPagePort>(() => {
    ...
    const guard = () => { if (identity !== current.current.identityVersion || device !== current.current.deviceId) throw new Error('Geräteverbindung wurde geändert.'); };
    ...
    return {
      list: async () => { const result = await query({ operation: 'list' }); if (!('index' in result)) throw new Error('Liste fehlt.'); return result.index; },
      detail: async id => { const result = await query({ operation: 'detail', id }); if (!('entry' in result)) throw new Error('Eintrag fehlt.'); return result; },
      sequence: async writerId => { const result = await query({ operation: 'writer', writerId }); if (!('sequence' in result)) throw new Error('Abgleichstand fehlt.'); return result.sequence; },
      ...
      recording: {
        ...
        prepare: async documentId => { const result = await dictation({ operation: 'prepare', documentId }, crypto.randomUUID()); if (!('jobId' in result)) throw new Error('Aufnahmeziel fehlt.'); return result; },
        ...
        query: async jobId => { const result = await dictation({ operation: 'query', jobId }); if (!('state' in result)) throw new Error('Diktatstand fehlt.'); return result.state; },
