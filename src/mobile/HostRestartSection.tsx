...
export function HostRestartSection({ host }: { host: MobileHost }): JSX.Element {
  ...
  useEffect(() => {
    mounted.current = true; let querying = false;
    const refresh = async () => {
  ...
  return <section className="m-settings-section" aria-labelledby="host-restart-title"><h3 id="host-restart-title">ADE auf dem PC</h3>
    {!state && !error && <p role="status">Host-Zustand wird geladen…</p>}
    {state && <><p>Version {state.version} · {state.restart === 'pending' ? 'Neustart vorbereitet' : 'Erreichbar'}</p>
      ...
      <button onClick={(event) => { event.currentTarget.focus(); setConfirm(true); }} disabled={!state.canRestart || state.blockers.length > 0
