...
export function MobileSessionSwitcher({ host, onSelect, onClose, fallbackId }: {
  ...
  return <Dialog title="Arbeit wechseln" onClose={onClose} fallbackId={fallbackId} className="session-switch-dialog">
    <SessionSwitcher items={mobileSessionNavigation(inventory.sessions, host.catalog)} loading={loading} error={error}
      online={host.status === 'online'} omitted={inventory.omitted} onRefresh={() => setRevision(value => value + 1)} onClose={onClose}
      onSelect={async id => {
        const session = inventory.sessions.find(item => item.id === id); if (!session) throw new Error('Sitzung nicht mehr vorhanden. Liste aktualisieren.');
        ...
        try {
          ...
          if (state.selected?.id !== id || expectedBranch !== undefined && state.selected.branch !== expectedBranch) throw new Error('Sitzung oder Branch hat sich geändert. Liste aktualisieren.');
          onSelect(target);
