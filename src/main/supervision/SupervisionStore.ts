...
export class SupervisionStore {
  ...
  constructor(private readonly path: string) {
    ...
    if (!validSupervisionState(state)) throw new Error(translate("ADE support storage is invalid and the original is retained."));
    this.state = state;
  }
  snapshot(): SupervisionState { return structuredClone(this.state); }
  save(next: SupervisionState): void {
    if (!validSupervisionState(next) || next.revision !== this.state.revision + 1) throw new Error(translate("Invalid supervision revision."));
    const bytes = JSON.stringify(next); if (Buffer.byteLength(bytes) > MAX_BYTES) throw new Error(translate("ADE supervision storage has reached its limit."));
    const verify = () => { const raw = this.read(); if ((raw === null ? null : supervisionDigest(raw)) !== this.fingerprint) throw new Error(translate("ADE support has been changed outside of this instance. Restart and check status.")); };
  ...
  private read(): string | null {
    ...
    try {
      const stat = fstatSync(fd); if (!stat.isFile() || stat.nlink !== 1 || stat.size > MAX_BYTES) throw new Error(translate("Insecure or too large ADE support storage."));
      ...
      if (length !== stat.size || after.size !== stat.size || after.mtimeMs !== stat.mtimeMs || after.ctimeMs !== stat.ctimeMs || named.dev !== stat.dev || named.ino !== stat.ino || named.nlink !== 1) throw new Error(translate("ADE support storage has been changed upon reading."));
      return raw;
