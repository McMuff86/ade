...
export class ProjectBranchService {
  ...
  async preview(workspaceId: string, action: ProjectBranchAction, owner: string): Promise<ProjectBranchPreview> {
    if (!validProjectBranchAction(action) || redactForWire(JSON.stringify(action), 4096) !== JSON.stringify(action)) throw new Error(translate("ade: Invalid branch action."));
    ...
    if (this.previews.size >= 50) throw new Error(translate("ade: Too many open git previews. Check again later."));
  ...
  async apply(previewId: string, owner: string, assertAuthorized: ProjectAuthorization = () => undefined): Promise<ProjectWorkspaceView> {
    return workspaceOperations.mutate(async () => {
      ...
      if (!stored || stored.owner !== owner || stored.view.expiresAt < this.now()) throw new Error(translate("ade: Branch preview expired or belongs to another session. Check again."));
      ...
      if (state.value.revision !== stored.revision) throw new Error(translate("ade: Branches or workspace changed since the preview. Check again."));
      const target = this.validateAction(state, action);
      if (target.head !== stored.targetHead) throw new Error(translate("ade: Branch base changed. Check again."));
      if (action.kind === 'open-worktree') {
        const worktree = target.worktree!;
        if (!stored.targetWorktree || worktree.identity !== stored.targetWorktree.identity || !sameHostPath(worktree.path, stored.targetWorktree.path)) throw new Error(translate("ade: Working copy has been changed."));
        assertAuthorized();
      ...
      if (action.kind === 'create' && action.separate) {
        ...
        if (existsSync(path)) throw new Error(translate("ade: New working copy already exists. Check again."));
        assertAuthorized();
        if (projectRootIdentity(parent) !== parentIdentity) throw new Error(translate("ade: Working copy root changed."));
        ...
        if (opened.branch !== action.name) throw new Error(translate("ade: New working copy could not be confirmed. Check project list."));
        return opened;
      }
      ...
      if (after.value.workspace.branch !== target.branch || after.value.head !== target.head || after.value.dirty) throw new Error(translate("ade: Branch change could not be confirmed cleanly. Check the current status."));
  ...
  private validateAction(state: Inspected, action: ProjectBranchAction): { branch: string; head: string | null; worktree?: WorktreeRecord } {
    if (action.kind === 'open-worktree') {
      ...
      if (!worktree?.view.available) throw new Error(translate("ade: Selected working copy is not available."));
      return { branch: worktree.view.branch, head: worktree.head || null, worktree };
    }
    this.assertBusy(state, action.kind === 'create' && action.separate);
    if (action.kind === 'create') {
      if (state.value.branches.some((branch) => branch.ref === `refs/heads/${action.name}`)) throw new Error(translate("ade: Branch already exists. Select existing branch."));
      const basis = action.baseRef ? state.value.branches.find((branch) => branch.ref === action.baseRef)?.head : state.value.head;
      if (action.baseRef && !basis) throw new Error(translate("ade: Base branch missing. Update view."));
      if (action.separate && !basis) throw new Error(translate("ade: Create a commit first for an additional working copy."));
      return { branch: action.name, head: basis ?? null };
    }
    const branch = state.value.branches.find((item) => item.ref === action.ref);
    if (!branch) throw new Error(translate("ade: Selected branch missing. Update view."));
    if (branch.worktreeId && !branch.current) throw new Error(translate("ade: This branch is open in another working copy. Select that working copy."));
    if (branch.kind === 'local') return { branch: branch.name, head: branch.head };
    const name = branch.name.slice(branch.name.indexOf('/') + 1);
    if (!name || state.value.branches.some((item) => item.ref === `refs/heads/${name}`)) throw new Error(translate("ade: A local branch already exists for this remote branch, select it or create a new branch."));
  ...
  private assertBusy(state: Inspected, separate: boolean): void {
    ...
    if (leased) throw new Error(translate("ade: Repository is occupied by a managed job."));
    if (state.operationInProgress) throw new Error(translate("ade: A git operation is running or must be completed."));
    if (separate) return;
    if (config.workspaceBindings.some((item) => sameHostPath(item.workspaceDir, path))) throw new Error(translate("ade: This working copy belongs to an agent profile. Create an independent working copy."));
    if (this.sessions().some((item) => item.status === 'running' && (item.executionBackend ?? 'native') === 'native' && item.workspaceDir && sameHostPath(item.workspaceDir, path))) throw new Error(translate("ade: A terminal session uses this workspace. End session or select additional working copy."));
    if (state.value.dirty) throw new Error(translate("ade: Unsecured changes. Commit first or choose an additional working copy."));
  ...
  private async inspect(workspaceId: string): Promise<Inspected> {
    ...
    const worktrees: WorktreeRecord[] = worktreesRaw.split('\0\0').filter(Boolean).map((record) => {
      ...
      if (!targetPath || /[\x00-\x1f]/.test(targetPath) || (ref && !validProjectBranchRef(ref)) || (head && !SHA.test(head))) throw new Error(translate("ade: Working copies could not be clearly read."));
      let identity = ''; let notice: string | null = null;
      try { identity = projectRootIdentity(targetPath); assertNoLinks(join(targetPath, '.git')); }
      catch { notice = translate("Working copy is unreachable or linked."); }
      if (fields.some((field) => /^(bare|locked|prunable)( |$)/.test(field))) notice = translate("Working copy is locked, stale or has no working directory. Check on the PC.");
    ...
    if (worktrees.length > 100) throw new Error(translate("ade: Maximum of 100 worktrees in this view."));
    const refLines = refsRaw.trimEnd().split(/\r?\n/).filter(Boolean);
    if (refLines.length > 200) throw new Error(translate("ade: Maximum of 200 branches in this view."));
    const branches: ProjectBranch[] = refLines.map((line) => {
      const [ref, head, symbolic] = line.split('\0');
      if (symbolic) return null;
      if (!ref || !head || !validProjectBranchRef(ref) || !SHA.test(head) || redactForWire(ref, 4096) !== ref) throw new Error(translate("ade: Branch list could not be clearly read."));
    ...
    if (branches.length > 200) throw new Error(translate("ade: Maximum of 200 branches in this view."));
    const current = worktrees.find((item) => item.view.current);
    if (!current) throw new Error(translate("ade: Current workspace is missing from the Git list."));
