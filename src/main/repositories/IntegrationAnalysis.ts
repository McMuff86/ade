...
export async function assertIntegrationGit(cwd: string, gitDirectory: string, common: string): Promise<void> {
  for (const root of new Set([gitDirectory, common])) {
    for (const name of ['config', 'config.worktree', 'index', 'HEAD', 'MERGE_HEAD', 'MERGE_MSG', 'ORIG_HEAD', 'refs', 'refs/heads/ade', 'refs/ade/integrations', 'objects', 'info', 'shallow']) {
      ...
      if (existsSync(path) && lstatSync(path).isFile() && lstatSync(path).nlink !== 1) integrationFail('Verknüpfte Git-Metadaten werden nicht verwendet.');
    }
  }
  const ref = await gitText(cwd, ['symbolic-ref', '--quiet', 'HEAD'], { accept: [1] });
  if (ref) {
    if (!ref.startsWith('refs/heads/') || ref.split('/').some((part) => !part || part === '.' || part === '..') || /[\\:\x00-\x1f]/.test(ref)) integrationFail('Ungültiger Git-Branch-Verweis.');
  ...
  if (drivers.split(/\r?\n/).some((line) => line && !/^filter\.lfs\.(?:clean|smudge|process)(?:\s|$)/.test(line))) integrationFail('Externe Git-Filter oder Merge-Treiber zuerst am PC prüfen. Diese Übernahme führt sie nicht aus.');
  ...
  for (const cached of [false, true]) {
    ...
    if (attributes.some((value, index) => index % 3 === 2 && !['unspecified', 'unset'].includes(value))) integrationFail('Dateien mit Git-Inhaltsfiltern (z. B. LFS) benötigen eine manuelle Übernahme.');
  }
}
...
export function integrationAvailability(config: AdeConfig, repository: Repository, paths: string[], sessions: SessionMeta[]): string[] {
  ...
  if (config.runWorkspaceLeases.some((item) => item.status === 'active' && sameHostPath(item.commonGitDir, repository.commonGitDir))) reasons.push('Ein verwalteter Auftrag belegt dieses Repository. Zuerst abschliessen.');
  if (sessions.some((item) => item.status === 'running' && (item.executionBackend ?? 'native') === 'native' && item.workspaceDir && paths.some((path) => sameHostPath(path, item.workspaceDir!)))) reasons.push('Ein Terminal verwendet Quelle, Ziel oder Arbeitskopie. Zuerst beenden.');
  return reasons;
}
...
export async function treeEntries(cwd: string, tree: string): Promise<Map<string, { oid: string; mode: string }>> {
  ...
  for (const entry of entries) {
    ...
    if (!match) integrationFail('Git-Dateibaum kann nicht sicher gelesen werden.');
...
export async function integrationStatus(cwd: string): Promise<{ raw: string; paths: string[]; conflicts: string[] }> {
  ...
  if (rows.length > 200) integrationFail('Mehr als 200 lokale Dateien. Änderungen zuerst in kleinere Übernahmen aufteilen.');
...
export async function integrationContents(cwd: string, paths: string[], head: string): Promise<IntegrationContent[]> {
  if (paths.length > 200) integrationFail('Maximal 200 Dateien pro Übernahme.');
  const tree = await treeEntries(cwd, head); const contents: IntegrationContent[] = []; let total = 0;
  for (const path of paths) {
    workbenchPath(path);
    if (redactForWire(path, 400) !== path) integrationFail('Ein Dateiname kann nicht sicher angezeigt werden. Am PC prüfen.');
    const mode = tree.get(path)?.mode ?? '100644';
    if (!['100644', '100755'].includes(mode)) integrationFail('Verknüpfungen und Submodule werden nicht übernommen.');
    const body = readIntegrationFile(join(cwd, path)); total += body?.length ?? 0;
    if (total > 16 * 1024 * 1024) integrationFail('Die ausgewählten Dateien überschreiten 16 MiB. Übernahme aufteilen.');
...
export async function analyzeIntegration(repository: Repository, source: IntegrationSourceRecord, projects: ProjectWorkspaceService, sessions: SessionMeta[], config: AdeConfig) {
  ...
  if (integrationDigest(sourceScope) !== source.identity) integrationFail('Quell-Workspace wurde ersetzt.');
  ...
  if (targetStatus.raw) blockers.push('Hauptworkspace enthält lokale Änderungen. Vorbereiten erst nach dem Sichern möglich.');
  if (sourceStatus.conflicts.length) blockers.push('Die Quelle enthält ungelöste Konflikte.');
  for (const scope of [targetScope, sourceScope]) {
    if (scope.branch === '(detached HEAD)') blockers.push('Quelle und Ziel benötigen einen ausgecheckten Branch.');
    for (const marker of ['MERGE_HEAD', 'rebase-merge', 'rebase-apply', 'CHERRY_PICK_HEAD', 'REVERT_HEAD']) if (existsSync(join(scope.workspace.gitDirectory, marker))) blockers.push('Quelle oder Ziel enthält eine laufende Git-Operation.');
  }
  const files: IntegrationFile[] = contents.map((file) => {
    ...
    return { path: file.path, kind: file.oid === null ? 'deleted' : !old ? 'new' : 'modified', local: sourceStatus.paths.includes(file.path),
      ...
      notice: present ? 'Bereits im Ziel enthalten.' : unchanged ? 'Keine Änderung gegenüber der gemeinsamen Basis.' : instructions ? 'Agent-Anweisungen oder Memory: nur bewusst auswählen.' : assessment === 'review' ? 'Auch im Ziel geändert. Inhalt und mögliche Konflikte prüfen.' : null };
  });
  ...
  if (integrationDigest(after) !== source.identity || await gitText(source.path, ['rev-parse', 'HEAD']) !== sourceHead || await gitText(repository.rootPath, ['rev-parse', 'HEAD']) !== targetHead) integrationFail('Workspace während der Prüfung geändert. Erneut prüfen.');
