...
export class IntegrationService {
  ...
  async query(query: IntegrationQuery, owner: string, authorize: ProjectAuthorization = () => undefined): Promise<IntegrationResult> {
    if (!validIntegrationQuery(query)) integrationFail('Ungültige Übernahmeabfrage.');
    return workspaceOperations.use(async () => {
      authorize();
      if (query.operation === 'diff') {
        const preview = this.previews.get(query.previewId);
        if (!preview || preview.owner !== owner || preview.view.expiresAt < this.now()) integrationFail('Vorschau abgelaufen oder für eine andere Sitzung.');
        ...
        if (state.fingerprint !== preview!.fingerprint) integrationFail('Dateien seit der Vorschau geändert. Erneut prüfen.');
        const content = state.contents.find((file) => file.path === query.path); if (!content) integrationFail('Datei nicht in dieser Vorschau.');
        ...
        const text = (body: Buffer) => body.includes(0) ? 'Binärdatei · keine Textvorschau.' : redactForWire(body.toString('utf8'), 16 * 1024);
      ...
      if (!source) integrationFail('Quelle ist nicht mehr erreichbar. Liste aktualisieren.');
      ...
      if (this.previews.size >= 50) integrationFail('Zu viele offene Vorschauen. Später erneut prüfen.');
  ...
  async command(command: IntegrationCommand, owner: string, authorize: ProjectAuthorization = () => undefined): Promise<IntegrationResult> {
    if (!validIntegrationCommand(command)) integrationFail('Ungültige Übernahmeaktion.');
  ...
  private repository(id: string): Repository {
    ...
    if (!repository?.verified || repository.executionBackend !== 'native') integrationFail('Ein geprüftes natives Repository auswählen.');
    return repository!;
  }
  private async sourceAnalysis(repository: Repository, sourceId: string): Promise<Analysis> {
    ...
    if (!source) integrationFail('Quell-Workspace wurde geändert oder entfernt. Erneut prüfen.');
    ...
    try {
      ...
      for (const file of contents) {
        if (file.body === null) await integrationGit(cwd, ['--literal-pathspecs', 'update-index', '--force-remove', '--', file.path], { index });
          ...
          if (oid !== file.oid) integrationFail('Dateifilter wurden während der Prüfung geändert.');
  ...
  private async prepare(command: Extract<IntegrationCommand, { operation: 'prepare' }>, owner: string, authorize: ProjectAuthorization): Promise<IntegrationReport> {
    ...
    if (!preview || preview.owner !== owner || preview.view.expiresAt < this.now()) integrationFail('Vorschau abgelaufen oder für eine andere Sitzung. Erneut prüfen.');
    const repository = this.repository(preview!.view.repositoryId); authorize({ repositoryId: repository.id });
    const state = await this.sourceAnalysis(repository, preview!.view.sourceId); authorize();
    if (state.fingerprint !== preview!.fingerprint) integrationFail('Quelle oder Ziel seit der Vorschau geändert. Erneut prüfen.');
    if (state.blockers.length) integrationFail(state.blockers.join(' '));
    if (command.paths.some((path) => !state.files.some((file) => file.path === path && file.selectable))) integrationFail('Dateiauswahl ist nicht mehr gültig.');
    ...
    try {
      ...
      if (again.fingerprint !== record.sourceFingerprint || again.blockers.length) integrationFail('Quelle oder Ziel während der Sicherung geändert.');
      record.snapshot = await gitText(repository.rootPath, [...IDENTITY, 'commit-tree', tree, '-p', state.base], { input: 'ADE: ausgewählte Quelländerungen sichern\n' });
      ...
      if (existsSync(path)) integrationFail('Integrations-Arbeitskopie ist bereits vorhanden.');
      authorize(); if (projectRootIdentity(parent) !== identity) integrationFail('Workspace-Stamm wurde ersetzt.');
      ...
      if (merged.code && !(await integrationStatus(path)).conflicts.length) integrationFail('Zusammenführung wurde unterbrochen. Arbeitskopie prüfen.');
  ...
  private async reviewState(record: IntegrationRecord) {
    if (!record.report.workspaceId) integrationFail('Vorbereitung ohne Arbeitskopie unterbrochen. Neue Vorschau erstellen; Sicherung bleibt erhalten.');
    const scope = await this.projects.resolve(record.report.workspaceId!); const cwd = scope.workspace.workspaceDir;
    if (cwd !== this.reviewPath(record) || scope.branch !== record.report.branch) integrationFail('Integrations-Arbeitskopie oder Branch wurde geändert.');
    ...
    if ((await integrationGit(cwd, ['merge-base', '--is-ancestor', record.report.targetHead, head], { accept: [1] })).code) integrationFail('Arbeitskopie basiert nicht mehr auf dem geprüften Ziel.');
    const mergeHead = await gitText(cwd, ['rev-parse', '--verify', '--quiet', 'MERGE_HEAD'], { accept: [1] });
    if (!record.snapshot || mergeHead !== record.snapshot && (await integrationGit(cwd, ['merge-base', '--is-ancestor', record.snapshot, head], { accept: [1] })).code) integrationFail('Vorbereitete Zusammenführung fehlt oder wurde abgebrochen. Eine neue Übernahme vorbereiten.');
    ...
    if (status.conflicts.length || markers.length) blockers.push('Konflikte bearbeiten und in Git als aufgelöst markieren.');
    for (const marker of ['rebase-merge', 'rebase-apply', 'CHERRY_PICK_HEAD', 'REVERT_HEAD']) if (existsSync(join(scope.workspace.gitDirectory, marker))) blockers.push('Laufende Git-Operation in der Arbeitskopie abschliessen.');
  ...
  private async report(record: IntegrationRecord): Promise<IntegrationReport> {
    ...
    try {
      ...
      if (integrationDigest(target) !== record.targetIdentity || await gitText(repository.rootPath, ['rev-parse', 'HEAD']) !== report.targetHead
        || (await integrationStatus(repository.rootPath)).raw) report.blockers.push('Zielstand wurde geändert oder enthält lokale Dateien. Neue Übernahme vorbereiten.');
  ...
  private async startTests(id: string, authorize: ProjectAuthorization): Promise<IntegrationResult> {
    ...
    return workspaceOperations.mutate(async () => {
      ...
      if (record.report.phase === 'integrated' || this.jobs.has(id)) integrationFail('Übernahme bereits abgeschlossen oder Prüfung läuft.');
        ...
        void workspaceOperations.use(async () => {
          try {
            ...
            for (let index = 0; index < plan.recipes.length; index++) {
              authorize(); if (controller.signal.aborted) integrationFail('Prüfung unterbrochen.');
            ...
            if (after.fingerprint !== state.fingerprint || after.blockers.length) integrationFail('Arbeitskopie wurde während der Prüfungen geändert oder belegt. Erneut prüfen.');
            ...
            if (!passed) record.report.blockers = ['Projektprüfung fehlgeschlagen. Arbeitskopie korrigieren und erneut prüfen.'];
          } catch (error) {
            record.report.phase = 'interrupted'; record.report.blockers = [redactedWireMessage(error, 1000)];
            for (const check of record.report.checks) if (check.status === 'running' || check.status === 'pending') { check.status = 'failed'; check.output = 'Prüfung unterbrochen.'; }
  ...
  private async integrate(record: IntegrationRecord, revision: string, message: string, authorize: ProjectAuthorization): Promise<IntegrationReport> {
    if (record.report.phase === 'integrated' || this.jobs.has(record.report.id)) integrationFail('Übernahme abgeschlossen oder Prüfung läuft.');
    const report = await this.report(record);
    if (report.revision !== revision || !report.tested || report.blockers.length || !record.testedTree || !record.report.checks.every((check) => check.status === 'passed')) integrationFail('Keine gültige Freigabe: Arbeitskopie, Ziel und Prüfungen erneut kontrollieren.');
    if (redactForWire(message, 2000) !== message) integrationFail('Commit-Nachricht enthält geschützte Angaben.');
    ...
    if (source.fingerprint !== record.sourceFingerprint || source.blockers.length) integrationFail('Quelle oder Ziel seit der Sicherung geändert. Neue Übernahme vorbereiten.');
    const state = await this.reviewState(record); authorize();
    if (state.fingerprint !== record.testedState || state.blockers.length) integrationFail('Geprüfte Arbeitskopie wurde geändert.');
    ...
    if (integrationDigest(target) !== record.targetIdentity || await gitText(repository.rootPath, ['rev-parse', 'HEAD']) !== record.report.targetHead
      || (await integrationStatus(repository.rootPath)).raw || blockers.length) integrationFail('Ziel inzwischen geändert oder belegt. Geprüfter Commit bleibt auf dem Integrations-Branch erhalten.');
    if (await gitText(state.cwd, ['rev-parse', `${record.reviewCommit}^{tree}`]) !== record.testedTree) integrationFail('Commit entspricht nicht dem geprüften Dateibaum.');
    authorize(); await integrationGit(repository.rootPath, ['merge', '--ff-only', '--no-overwrite-ignore', '--', record.reviewCommit!]);
    if (await gitText(repository.rootPath, ['rev-parse', 'HEAD']) !== record.reviewCommit || (await integrationStatus(repository.rootPath)).raw) integrationFail('Übernahme konnte nicht sauber bestätigt werden. Zustand am PC prüfen.');
