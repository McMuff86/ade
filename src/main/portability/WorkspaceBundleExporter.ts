...
export function exportWorkspaceBundle(
  ...
  const agents = exportableAgents.map((agent) => {
    ...
    return {
      ...
      ...(agent.ollamaMode ? { ollamaMode: agent.ollamaMode } : {}),
      ...(agent.ollamaHarness ? { ollamaHarness: agent.ollamaHarness } : {}),
      ...(agent.claudeModel ? { claudeModel: agent.claudeModel } : {}),
  ...
  const agentTemplates = config.agentTemplates.map((template) => {
    ...
    return {
      ...
      ...(template.ollamaMode ? { ollamaMode: template.ollamaMode } : {}),
      ...(template.ollamaHarness ? { ollamaHarness: template.ollamaHarness } : {}),
      ...(template.claudeModel ? { claudeModel: template.claudeModel } : {}),
