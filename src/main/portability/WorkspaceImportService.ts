...
function importedConfig(
  ...
  const agents: Agent[] = plan.agents.filter((item) => item.status === 'ready').map((item) => {
    ...
    return {
      ...
      ...(source.ollamaMode ? { ollamaMode: source.ollamaMode } : {}),
      ...(source.ollamaHarness ? { ollamaHarness: source.ollamaHarness } : {}),
      ...(source.claudeModel ? { claudeModel: source.claudeModel } : {}),
