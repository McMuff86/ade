...
import { validSpeechSelection, validSpeechTarget, validSpeechPreset, validVoiceId, type SpeechPreset, type SpeechAudio, type SpeechTarget } from '../../shared/speech';
...
export const validSpeechCommand = (value: unknown): value is MobileSpeechCommand => {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return false;
  const input = value as Record<string, unknown>;
  return input.operation === 'select' && Object.keys(input).length === 3 && validSpeechSelection({ target: input.target, voiceId: input.voiceId })
    || input.operation === 'test' && Object.keys(input).every(key => ['operation', 'target', 'voiceId', 'preset'].includes(key))
      && validSpeechTarget(input.target) && validVoiceId(input.voiceId) && (!Object.hasOwn(input, 'preset') || validSpeechPreset(input.preset));
};
...
export class RemoteSpeechService {
  ...
  async test(owner: string, target: SpeechTarget, voiceId: string, authorize: () => void, preset?: SpeechPreset): Promise<{ testId: string }> {
