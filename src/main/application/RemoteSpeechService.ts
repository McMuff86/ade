...
import { validSpeechSelection, validSpeechTarget, validVoiceId, type SpeechAudio, type SpeechTarget } from '../../shared/speech';
...
export const validSpeechCommand = (value: unknown): value is MobileSpeechCommand => {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return false;
  const input = value as Record<string, unknown>;
  return Object.keys(input).length === 3 && (input.operation === 'select' && validSpeechSelection({ target: input.target, voiceId: input.voiceId })
    || input.operation === 'test' && validSpeechTarget(input.target) && validVoiceId(input.voiceId));
};
