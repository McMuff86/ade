...
export class CoordinatorActionService {
  ...
  private save(action: StoredCoordinatorAction, command?: CoordinatorActionCommand): void {
    ...
    if (command && !state.commands.some(c => c.id === command.commandId)) {
      if (state.commands.length >= 1024) throw new Error('ADE-Aktionsquittungen haben ihr Speicherlimit erreicht.');
  ...
  private find(conversationId: string, actionId: string): StoredCoordinatorAction {
    ...
    if (!action) throw new Error('Dieser Auftrag gehört nicht zum gewählten ADE-Gespräch.'); return action;
  }
  private project(input: CoordinatorActionInput) {
    ...
    if (!project) throw new Error('Betreutes Projekt ist nicht verfügbar.');
    if (input.kind === 'task' && project.mode !== 'coordinate') throw new Error('Für Projektaufträge die ADE-Betreuung dieses Projekts auf Koordinieren stellen und ein neues Gespräch beginnen.');
    return project;
  }
  private worker(input: CoordinatorActionInput): string | null {
    ...
    if (!agent || agent.runtime !== 'codex' || agent.customCommand?.trim() || agent.homeExecutionBackend && agent.homeExecutionBackend !== 'native'
      || !agent.codexModel || !agent.codexReasoningEffort) throw new Error('Projektauftrag braucht ein natives Codex-Profil mit Modell und Reasoning.');
  ...
  private authorize(action: StoredCoordinatorAction, requireOpen: boolean): void {
    ...
    if (project.repositoryId !== action.repositoryId || !repo || action.input.kind === 'task' && repo.executionBackend !== 'native'
      || this.worker(action.input) !== action.workerDigest) throw new Error('Projekt oder Codex-Profil wurde geändert. Einen neuen Auftrag vorbereiten.');
  }
  propose(input: unknown, source: CoordinatorActionSource, context: CodexToolContext): CoordinatorActionSummary {
    if (!validCoordinatorActionInput(input)) throw new Error('Ungültiger ADE-Aktionsvorschlag.');
    ...
    if (previous) { if (previous.sourceDigest !== sourceDigest) throw new Error('ADE-Werkzeugaufruf wurde mit geändertem Inhalt wiederholt.'); return this.summary(previous); }
    if (this.store.snapshot().actions.length >= 256) throw new Error('ADE-Auftragsspeicher ist voll. Bestehende Aufträge bleiben erhalten.');
  ...
  private recovered(action: StoredCoordinatorAction): StoredCoordinatorAction {
    ...
    const unavailable = (): StoredCoordinatorAction => ({ ...action, state: 'uncertain', error: 'Gespeicherter Auftragsbeleg ist nicht mehr verfügbar oder widersprüchlich. Es wird nichts erneut gestartet.' });
    ...
    return this.inFlight.has(action.id) ? action : { ...action, state: 'uncertain', error: action.error || 'Auftragsausführung nicht bestätigt. Es wird nichts erneut gestartet.' };
  }
  private summary(stored: StoredCoordinatorAction): CoordinatorActionSummary {
    ...
    return { id: a.id, conversationId: a.conversationId, turnId: a.turnId, kind: a.input.kind, projectId: a.input.projectId,
      projectName: config.repositories.find(r => r.id === a.repositoryId)?.name ?? 'Nicht verfügbares Projekt',
      agentName: a.input.kind === 'task' ? config.agents.find(p => p.id === (a.input as { agentId: string }).agentId)?.name ?? 'Entferntes Profil' : null,
  ...
  work(conversationId: string, actionId: string): CoordinatorActionWork {
    ...
    if (a.input.kind !== 'task' || !a.runId || !a.taskId) throw new Error('Dieser Auftrag hat noch keinen bestätigten Run.');
  ...
  command(input: CoordinatorActionCommand, callerAuthorize: () => void = () => undefined): Promise<CoordinatorActionReceipt> {
    if (!validCoordinatorActionCommand(input)) return Promise.reject(new Error('Ungültiger ADE-Aktionsbefehl.'));
    const receipt = this.store.snapshot().commands.find(c => c.id === input.commandId);
    if (receipt && receipt.digest !== conversationFingerprint(input)) return Promise.reject(new Error('ADE-Aktionsbefehl wurde mit geänderter Eingabe wiederholt.'));
    callerAuthorize(); const action = this.find(input.conversationId, input.actionId);
    const existing = this.inFlight.get(action.id);
    if (existing) {
      if (input.operation !== 'confirm') return Promise.reject(new Error('Dieser Auftrag wird bereits ausgeführt.'));
  ...
  private async execute(input: CoordinatorActionCommand, callerAuthorize: () => void): Promise<CoordinatorActionReceipt> {
    const receipt = this.store.snapshot().commands.find(c => c.id === input.commandId);
    if (receipt && receipt.digest !== conversationFingerprint(input)) throw new Error('ADE-Aktionsbefehl wurde mit geänderter Eingabe wiederholt.');
    ...
    if (action.state !== 'proposed') {
      if (input.operation === 'dismiss' && action.state !== 'dismissed' || input.operation === 'confirm' && action.state === 'dismissed') throw new Error('Dieser Vorschlag wurde bereits anders entschieden.');
      if (action.state === 'dispatching' || action.state === 'uncertain') throw new Error('Ausführung bleibt unbestätigt. Keine automatische Wiederholung.');
    ...
    try {
      ...
      if (action.input.kind === 'handoff') this.deps.supervision.command(action.handoffCommand!);
        ...
        const submission = await this.deps.submit({ agentId: action.input.agentId, repositoryId: action.repositoryId, prompt: action.input.prompt,
          name: 'Codex-Projektauftrag', allowQuestions: true, commandId: action.commandId }, authorize, child => {
